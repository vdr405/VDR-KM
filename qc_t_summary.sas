...
%macro spleen(popid= ITTFL , partn=2,prod= , trtc= trt01p );
proc sort data= derived.admf07d 
    out=adim(where=((anl01fl eq "Y" or ablfl eq "Y" ) and &popid eq "Y" and partn eq &partn and strip(upcase(paramcd)) eq "TSS" ));
...
data avisit(keep= variable avisit: srt paramcd param );
 set final;
    variable= "MFSAF v3.0 Total Symptom Score at "||strip(avisit);
    if first.avisit;
    by paramcd param avisitn avisit;
run;
/**/
...
*PROC COMPARE DATA=PROD COMPARE=COMP  Out=DIFF OUTNOEQUAL OUTCOMP OUTDIFF OUTBASE LISTALL; RUN;
/*** assign sysinfo value into a global macro var ***/ 
/*%global compinfo1 ; */
/*%let compinfo1  = &sysinfo;*/
%comp_report(BASE=  PROD, COMPARE= COMP, dsname=&PROD);

ODS RTF CLOSE;


%mend;
%spleen(popid= ITTFL,partn=2,prod=t_summf1);
